...
//以下代码针对的是 多个进程在等待同一资源的情况 这种是可被打断的 而上面的代码是不可打断的
//实际上所用的数据结构就是链表 只不过不是平常意义的链表 
//他的结构是直接在内存中进行维护的。
...
repeat:	current->state = TASK_INTERRUPTIBLE;
	...
	/*end print sleep*/
	schedule();
	//如果当前的不是等待队列的头指针 那么就先将头指针给释放掉 再将当前的阻塞 维持数据结构的定义
	//这个预期叫做等待队列不如叫做等待栈 等待链表
	...
	*p=tmp;//此处确实应该是tmp NULL的话就把后面的链表都给丢了
	if (tmp) {//如果tmp不是最后一个 也就是说还存在等待的任务 就一并唤醒
		/*wake up*/
		fprintk(3, "%d\tJ\t%ld\n", tmp->pid, jiffies);
		/*end print*/
...
void do_timer(long cpl)
{
	...
	if (next_timer) {//如果存在软驱操作定时器 则将链表第一个定时器的值减一 如果已经等于0 则调用相应处理程序
		next_timer->jiffies--;
	...
	//这里减少了当前任务的时间片 如果当前任务还存在时间 就直接返回
	if ((--current->counter)>0) 
		return;
	// 否则将其置为0 安全起见
	current->counter=0;
	//如果是内核程序 其不依赖于counter进行调度
	if (!cpl) 
		return;
	//用户程序再进行调度
	schedule();
...
/* Clear NT, so that we won't have troubles with that later on */
	...
	ltr(0);
	lldt(0);
	//set 8253 mode time interrupt is LATCH
